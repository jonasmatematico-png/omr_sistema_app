...
class _TelaCorrecaoLoteState extends State<TelaCorrecaoLote> {
          ...
          children: [
            IconButton(
              icon: Icon(
                ...
                color: f['status'] == 'salvo'
                    ? Colors.red
                    : Colors.grey.shade300,
              ),
              tooltip: f['status'] == 'salvo'
                  ? 'Gerar Relatório PDF'
                  : 'Salve a nota primeiro',
              onPressed: f['status'] == 'salvo'
                  ? () => _abrirPdfAluno(f['id_aluno'])
                  : null,
            ),
            IconButton(
              icon: Icon(
              ...
              tooltip: f['status'] == 'salvo'
                  ...
                  : 'Salve a nota primeiro',
              onPressed: f['status'] == 'salvo'
                  ? () => _compartilharComPais(idxFila, f)
                  : null,
            ),
          ],
        ),
  ...
  // 📤 Menu de compartilhamento com os pais
  Future<void> _compartilharComPais(int idx, Map<String, dynamic> f) async {
        ...
        child: Column(
          ...
          children: [
            ...
            ListTile(
              ...
              subtitle: const Text(
                'Relatório completo com comentários',
            ...
            ListTile(
              ...
              title: const Text('📷 Foto da prova + nota'),
              subtitle: const Text(
                'Folhas fotografadas com a nota na mensagem',
                style: TextStyle(fontSize: 11),
              ),
              onTap: () {
                Navigator.pop(ctx);
            ...
            ListTile(
              ...
              title: const Text('💬 Só a mensagem com a nota'),
              subtitle: const Text(
                'Texto rápido com nota e nível',
                style: TextStyle(fontSize: 11),
              ),
              onTap: () {
                Navigator.pop(ctx);
                Share.share(_mensagemPais(idx, f));
              },
            ),
            const SizedBox(height: 8),
          ],
  ...
  // 💬 Monta a mensagem pros pais (edite seu nome no final!)
  String _mensagemPais(int idx, Map<String, dynamic> f) {
    final soma = _somaNotasEditadas(idx);
    final nota10 = _somaValores > 0 ? (soma * 10 / _somaValores) : soma;
    String nivel = 'Abaixo do Básico';
    if (nota10 >= 8)
      nivel = 'Avançado';
      ...
      nivel = 'Básico';
    return '📚 *Feedback de Avaliação*\n\n'
        'Aluno(a): *${f['nome_aluno']}*\n'
        'Nota: *${nota10.toStringAsFixed(1)} / 10,0*\n'
        'Nível: *$nivel*\n\n'
        'Qualquer dúvida, estou à disposição! 😉\n\n'
        '— Prof. Jonas'; // ← edite seu nome aqui!
  }

  // 📷 Compartilha as fotos da prova + mensagem com a nota
  Future<void> _shareFotos(int idx, Map<String, dynamic> f) async {
    final caminhos = (f['caminhos'] as List?)?.cast<String>() ?? [];
    if (caminhos.isEmpty) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('⚠️ Sem fotos disponíveis nesta prova.'),
          ),
        );
      }
      return;
    }
    final files = caminhos.map((c) => XFile(c)).toList();
    await Share.shareXFiles(files, text: _mensagemPais(idx, f));
  }

  // 📄 Baixa o PDF e compartilha como arquivo
  Future<void> _sharePdf(int idx, Map<String, dynamic> f) async {
    if (f['id_aluno'] == null || avalId == null) return;
    final appState = Provider.of<AppState>(context, listen: false);
    final url =
        '${appState.ipServidor}/api/relatorio/aluno/${f['id_aluno']}/$avalId';
    try {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('📄 Baixando relatório...')),
        );
      }
      final resp = await http
          .get(Uri.parse(url))
      ...
      if (resp.statusCode != 200) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('⚠️ Servidor respondeu ${resp.statusCode}')),
          );
        }
        return;
      }
      final dir = await Directory.systemTemp.createTemp('relatorio_share');
      final nomeSeguro = '${f['nome_aluno']}'
          .replaceAll(RegExp(r'[^a-zA-Z0-9_\- ]'), '')
          .trim();
      final file = File('${dir.path}/relatorio_$nomeSeguro.pdf');
      await file.writeAsBytes(resp.bodyBytes);
      await Share.shareXFiles([XFile(file.path)], text: _mensagemPais(idx, f));
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(
          context,
